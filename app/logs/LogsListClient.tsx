...
export default function LogsListClient({ initialLogs }: { initialLogs: LogType[] }) {
  ...
  const getLogBadge = (label: string) => {
    ...
    if (lowerLabel.includes("créé")) return { color: "bg-green-100 text-green-700", text: "Création" };
    if (lowerLabel.includes("mis à jour") || lowerLabel.includes("modifié") || lowerLabel.includes("renommé")) return { color: "bg-orange-100 text-orange-700", text: "Modif." };
    if (lowerLabel.includes("supprimé")) return { color: "bg-red-100 text-red-700", text: "Suppr." };
    return { color: "bg-gray-100 text-gray-600", text: "Autre" };
  };
  ...
  return (
    <div>
      ...
      {selectedLogIds.length > 0 && (
        <div className="mb-4 p-4 bg-[#F4F7F5] border border-[#E2EAE5] rounded-xl flex items-center justify-between">
          <span className="text-sm font-bold text-[#0F5E3D]">
      ...
      <div className="mb-6">
        <div className="flex items-center w-full md:w-72 bg-white border border-[#E2EAE5] rounded-xl px-3 py-2 shadow-sm transition-all focus-within:ring-2 focus-within:ring-[#047857]/20 focus-within:border-[#047857]">
          {/* Icône de recherche */}
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#A3B8AC]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <input
            ...
            className="w-full ml-2 bg-transparent text-sm text-[#1E2E24] placeholder:text-[#A3B8AC] focus:outline-none"
          />
        </div>
      </div>
        <table className="w-full border-collapse text-left text-sm text-[#1E2E24]">
          ...
          <tbody className="divide-y divide-[#F0F4F1]">
            ...
            {paginatedLogs.map((log) => {
              ...
              return (
                <tr key={log.logsId} className={`hover:bg-[#F9FAF9] transition-colors ${selectedLogIds.includes(log.logsId) ? 'bg-[#F4F7F5]' : ''}`}>
                  ...
                  <td className="py-3 px-4 font-mono text-xs text-[#8A9A8E]">#{log.logsId}</td>
                  ...
                  <td className="py-3 px-4 text-[#1E2E24]">{log.label}</td>
                  <td className="py-3 px-4 text-center">
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[100] flex items-center justify-center p-6 animate-fade-in">
          <div className="bg-white p-6 rounded-2xl max-w-sm w-full shadow-2xl border border-slate-200">
            ...
            <h3 className="text-lg font-bold text-[#1E2E24] text-center mb-2">
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-bold rounded-xl transition-colors cursor-pointer">Annuler</button>
      ...
      {toast && (
        <div className={`fixed bottom-10 right-10 border-l-4 p-4 rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] flex items-center gap-3 z-[20000] animate-fadeIn transition-all bg-white cursor-pointer ${
          ...
          <div>
            <h4 className="text-sm font-bold text-[#1E2E24]">{toast.type === "success" ? "Succès" : "Erreur"}</h4>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-4 mt-6 items-center">
          <button 
            ...
            className="px-3 py-1 bg-white border border-[#E2EAE5] rounded-lg disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed"
          >Précédent
          </button>
          <span className="text-sm text-[#53665A]">Page {currentPage} / {totalPages}</span>
          <button 
