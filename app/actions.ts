...
export async function getStudentsByClass(classId: number) {
  ...
  return students.map((student) => {
    const values = student.grades.map((g) => Number(g.value)); // FIX
    const globalAverage =
      values.length > 0
...
export async function addDebugUser(
  mail: string,
  password: string,
  firstname: string,
  surname: string,
  ...
  try {
    ...
    console.error(
      "Erreur lors de la création de l'utilisateur de debug :",
      error
    );
...
export async function addDebugStudent(
  classId: number | null,
  firstname: string,
  surname: string
) {
...
export async function addGroup(label: string, studentIds: bigint[]) {
  try {
    ...
    console.error(
      "Erreur lors de la création du groupe et de ses assignations :",
      error
    );
...
export async function addClass(label: string, studentIds: bigint[]) {
  try {
    ...
    console.error(
      "Erreur lors de la création de la classe et de l'assignation des étudiants :",
      error
    );
  ...
  try {
    return await prisma.$transaction([
      ...
      prisma.teacherAssignments.createMany({
        data: teacherIds.map((id) => ({ subjectId, teacherId: id })),
      }),
    ]);
  } catch (error) {
    console.error("Erreur lors de la mise à jour des assignements :", error);
    throw new Error(
      "Impossible de mettre à jour les assignements des enseignants."
    );
  }
}

export async function updateSubjectAssignments(
  studentIds: bigint[],
  subjectId: number
) {
  try {
    return await prisma.$transaction([
      ...
      prisma.subjectAssignments.createMany({
        data: studentIds.map((id) => ({ subjectId, studentId: id })),
      }),
    ]);
  } catch (error) {
    console.error("Erreur lors de la mise à jour des assignements :", error);
  ...
  try {
    return await prisma.$transaction([
      ...
      prisma.studentAssignments.createMany({
        data: studentIds.map((id) => ({ groupId, studentId: id })),
      }),
    ]);
  } catch (error) {
    console.error("Erreur lors de la mise à jour des assignements :", error);
    throw new Error(
      "Impossible de mettre à jour les assignements des étudiants."
    );
  }
}
...
export async function loginAction(
  ...
  try {
    ...
    await prisma.session.create({
      data: { token: sessionToken, userId: user.userId, expiresAt },
    });
...
export async function getDropdownData() {
  try {
    ...
    console.error(
      "Erreur lors de la récupération des données pour les menus:",
      error
    );
...
export async function createAssessment(data: {
  ...
  try {
    ...
    return {
      success: true,
      assessmentId: newAssessment.assessmentId.toString(),
    };
  } catch (error) {
...
/**
 * Ajoute ou met à jour la note et le feedback d'un étudiant
 */
...
export async function sendResetCodeEmail(targetEmail: string) {
  try {
    ...
    if (!user) {
      return {
        success: false,
        error: "Aucun compte n'est associé à cette adresse.",
      };
    }
    ...
    try {
      const resetEnregistre = await prisma.passwordReset.upsert({
        ...
        update: { resetCode: code, validityTime: expiresAt },
        create: { userId: user.userId, resetCode: code, validityTime: expiresAt },
      });
      console.log(
        "-> [BDD SUCCESS] Ligne enregistrée avec succès :",
        resetEnregistre
      );
    } catch (prismaError) {
      console.error(
        "-> [BDD CRASH] Prisma n'a pas pu écrire dans PostgreSQL :",
        prismaError
      );
              ...
              <table role="presentation" width="100%" max-width="480" cellspacing="0" cellpadding="0" border="0" style="max-width: 480px; width: 100%; background-color: #ffffff; border-radius: 16px; border: 1px solid #E2EAE5; box-shadow: 0 4px 20px rgba(18,38,30,0.02); overflow: hidden;">
                <tr>
                  <td style="padding: 32px 32px 20px 32px; text-align: center;">
                    <h1 style="margin: 0; font-size: 24px; font-weight: 800; color: #12261E;">Junia'lytics</h1>
                ...
                <tr>
                  <td style="padding: 32px;">
                    <p style="margin: 0 0 16px 0; font-size: 14px; line-height: 1.6; color: #1E2E24;">Bonjour,</p>
                    ...
                    <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin-bottom: 24px;">
                      <tr>
                        <td style="vertical-align: middle; padding-right: 8px;"><span style="font-size: 16px;">⏱️</span></td>
                        <td style="vertical-align: middle;">
                          <p style="margin: 0; font-size: 12px; font-weight: 600; color: #B91C1C;">Ce code secret est strictement confidentiel et expirera dans 15 minutes.</p>
                        </td>
                      </tr>
                    </table>
                ...
                <tr>
                  <td style="padding: 0 32px 32px 32px; text-align: center;">
                    <p style="margin: 0; font-size: 11px; color: #8A9A8E;">Junia'lytics — Plateforme d'analyse académique.</p>
                  </td>
                </tr>
              </table>
            </td>
...
export async function verifyResetCode(
  ...
  try {
    const resetRequest = await prisma.passwordReset.findFirst({
      where: { user: { mail: targetEmail } },
    });

    if (!resetRequest) {
      return {
        success: false,
        error: "Aucune demande de récupération n'est active pour ce compte.",
      };
    }

    if (new Date() > resetRequest.validityTime) {
      return {
        success: false,
    ...
    return {
      success: false,
      error: "Une erreur système est survenue lors de la vérification.",
    };
  }
}

export async function updatePasswordAndCleanUp(
  targetEmail: string,
  ...
  try {
    ...
    // Remplace par un vrai hash si tu utilises bcrypt/argon2
    const passwordToStore = newPasswordRaw;
    ...
    await prisma.$transaction([
      ...
      // prisma.passwordReset.delete({ where: { userId: user.userId } }),
    ]);

    console.log(
      `[BDD] Mot de passe mis à jour avec succès pour ${targetEmail}.`
    );
    return { success: true };
  } catch (error) {
    ...
    return {
      success: false,
      error: "Impossible d'enregistrer le nouveau mot de passe.",
    };
  }
}